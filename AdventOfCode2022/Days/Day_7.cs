...
public class Day_7
{
    ...
    private static void ProcessComands()
    {
        ...
        for (int i = 0; i < commands.Length; i++)
        {
            ...
            {
                if (commands[i][2] == 'l')
                    dicionaryFolders.Add(current, 0);

                else if (commands[i][2] == 'c')
                {
                    if (commands[i][5] == '.')
                        current = current.Substring(0, current.LastIndexOf('|'));

                    else
                        current += "|" + commands[i].Split(" ")[2];
                }
            }
            ...
            else
            {
                if (commands[i][0] != 'd')
                {
                    ...
                    while (temp != "")
                    {
                        dicionaryFolders[temp] += Int32.Parse(commands[i].Split(" ")[0]);
	...
	{
        ProcessComands();
        return dicionaryFolders.Where(x => x.Value <= 100000).Sum(x => x.Value);
	}
    ...
    public static int PartTwo()
    {
        ...
        int storageNeeded = 30000000 - (70000000 - dicionaryFolders["|/"]);
        dicionaryFolders.Remove("|/");
        return dicionaryFolders.Where(x => x.Value >= storageNeeded).OrderBy(x => x.Value).First().Value;
    }
}
